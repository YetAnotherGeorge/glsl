...
float GetDist(vec2 pos, vec2 mouse)
{
    float radius = 24.0;
    vec2 f0 = mouse;
    vec2 f1 = vec2(sin(u_time * 0.2) * 10.0, cos(u_time * 0.7) * 10.0);
    vec2 f2 = vec2(sin(u_time * 0.2 + 3.4) * 10.0, cos(u_time * 0.7 + 5.5) * 10.0);

    float df0 = distance(pos, f0);
    float df1 = distance(pos, f1);
    float df2 = distance(pos, f2);

    float d_center = df0 + df1 + df2;
    return smoothstep(0.9, 1.0, sin(d_center));
}
void main()
{
