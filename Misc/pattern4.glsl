...
vec3 Shader(vec2 uv)
{
    vec2 pnv = uv * 3.0; //vertical noise scale

    float nv = 0.0, mult = 0.5, multSum = 0.0;
    for(int i = 0; i < 7; i++)
    {
        nv += snoise(pnv) * mult;
        pnv = pnv * 2.0 + u_time * 0.3;
        multSum += mult;
        mult *= 0.5;
    }
    nv = abs(nv/multSum);

    return ColorRampLinear(nv, 
        0.1, vec3(abs(sin(u_time * 0.1)), abs(sin(u_time * 0.8 + 2.)), abs(sin(u_time))), 
        0.2, vec3(0.9804, 0.9765, 0.9765), 
        0.7, vec3(0.3294, 0.0431, 0.5647)
        );
    // vec3 col = ColorRampConstant(n, 
    //     0.3,    vec3(0.2118, 0.0235, 0.149),
    //     0.5,    vec3(0.229, 0.299, 0.186),
    //     0.7,    vec3(0.535, 0.561, 0.355),
    //     0.8,    vec3(0.2941, 0.0627, 0.1137)
    // );
}