...
vec3 GetCol(vec2 pos, vec2 mouse)
{
    float d_star = DStar(pos);
    float d_dot = DMovingDotGrid(pos, 1.0, Step(u_time, 0.5, 1.1) * 0.3);

    float d_star_glow = 1.0 / d_star;
    float d_dot_glow = 1.0 / d_dot;

    vec3 star_col = vec3(1.0, 1.0, 1.0) * smoothstep(0.2, 1.0, d_star_glow / LINE_WIDTH);
    vec3 dot_col = vec3((cos(u_time * 0.1 + pos.x * 0.1) + 1.0) * 0.5, (sin(u_time * 0.3 + 0.2 + pos.y * 0.1) + 1.0) * 0.5, 0.2) * 
        smoothstep(0.0, 1.0, d_dot_glow / (LINE_WIDTH * 0.5));
    vec3 col = star_col * (0.7 + dot_col) + dot_col;
    return col;
}
void main()
{
    ...
    gl_FragColor = vec4(GetCol(pos, mouse), 1.0);
}