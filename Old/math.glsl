...
//distort base = 0.2
float NoiseCurves_0(vec2 pos, float scale, float distort)
{
    vec2 pscaled = pos * scale;

    float a0 = 0.5;
    vec2 n0 = vec2((snoise(pscaled * a0) * 0.5 + 0.5));
    n0.y = abs(sin(n0.y) * n0.y);

    float n1 = (snoise(pscaled * 0.3 + n0 * distort) * 0.5 + 0.5);
    float n2 = (sin(n1 * M_PI * 6.5));
    return max(0.0, n2);
}
//
...
{
    return(NoiseCurves_0(pos, 1.0, 0.2));
}
void main()
{
